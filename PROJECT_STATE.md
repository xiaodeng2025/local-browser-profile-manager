...
本单元由用户明确授权，只在公开仓库自身的 `codex/public-r1` 分支工作，不合并、
cherry-pick 或推送私库历史，也不启动服务、浏览器、真实代理或真实网站。

范围严格限定为两组通用能力：

1. 每档案单个固定 HTTP/HTTPS 代理的 Basic 认证持久化启动。凭据继续由现有
   Windows 本地加密存储管理，通过 Playwright persistent-context 启动参数传递，
   不写入 Chromium 命令行；固定代理启动的就绪页使用 `about:blank`，避免把
   Manager 的本地就绪请求发往外部代理。SOCKS5 用户名密码认证不支持。
2. 每档案生成并持久化一个固定 uint32 fingerprint seed；启动参数覆盖全局同名
   参数。首次启动记录所用浏览器可执行文件的 SHA-256，后续不一致时 fail closed。
   所有公开 Profile 返回值只含 `fixed` 和执行文件锁定状态，不返回 seed 或哈希。

管理页只读显示“固定 / 待首次启动锁定”或“固定 / 浏览器版本已锁定”。这里的
“锁定”仅表示本地记录的可执行文件摘要一致，不表示浏览器一定消费该参数，也不
表示目标网站放行、反检测、跨浏览器版本身份持久、地区或网络隔离。

验证范围仅为本地单元测试与静态检查，使用临时目录、合成浏览器文件和虚构代理
域名；没有连接代理、读取个人档案、启动服务或浏览器。全量 unit 42/42 通过；
16 个 Python 文件只读编译、前端 JavaScript 语法和 `git diff --check` 通过。
待提交文件的敏感信息与范围审计未发现实际凭据、机器绝对路径、私库引用或范围外
文件。
